...
export async function createWarningIssue(item: BuriedItem, options: IssueOptions = {}): Promise<{ url?: string } | null> {
  if (options.dryRun) {
  ...
  try {
    ...
    console.warn('Failed to create issue:', (err as Error).message);
    return null;
  }
}

export default { createWarningIssue };
