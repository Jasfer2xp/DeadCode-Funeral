 ...
 */
function moduleSpecifiersFor(filePath, root) {
    ...
    const specifiers = [noExt, fromRoot];
 ...
 */
function specifierMatchesDefining(importSpecifier, consumerFile, definingSpecifiers, absRoot) {
    ...
    if (!importSpecifier.startsWith('.')) {
        ...
        return definingSpecifiers.some((s) => s === importSpecifier || s.endsWith('/' + importSpecifier));
    }
 ...
 * defining module.
 */
function analyzeConsumer(consumerFile, functionName, definingSpecifiers, absRoot) {
    let src;
    try {
    ...
    for (const imp of imports) {
        ...
        if (imp.namespaceAlias) {
            ...
            const nsUsageRe = new RegExp(`\\b${escapeRegExp(imp.namespaceAlias)}\\.${escapeRegExp(functionName)}\\b`, 'g');
            let mu;
        ...
        if (imp.defaultAlias && imp.named.length === 0) {
            ...
            const aliasRe = new RegExp(`\\b${escapeRegExp(imp.defaultAlias)}(?:\\.${escapeRegExp(functionName)})?\\b`, 'g');
            let mu;
 ...
 */
function wordBoundarySearch(file, functionName) {
    let src;
    try {
...
export function checkUsage(root, definingFile, functionName) {
    ...
    const allDetails = [];
    const seenKeys = new Set(); // deduplicate file:line
